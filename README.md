...
### 데코레이터 디자인 패턴
- 클레스에 추가적인 기능을 확장하고싶을때 사용하는 디자인 패턴
- 기반 컴포넌트, 데코레이터 컴포넌트를 동일한 클레스로부터 상속을 받고 데코레이터 컴포넌트는 추가적인 기능을 가진 데코레이터 에게 상속해주는 방식으로 구현함
- 사용은 new DecoA(new Decorator(new 기반컴포넌트)) 방식으로 생성해서 구현한 메소드를 사용


### 동기화
- 크리티컬 섹션에 하나의 메소드만 접근하도록 하는것
- syncronization 키워드를 사용하여 메소드 앞에 붙이거나, 블럭을 만들어서 동기화를 한다.
